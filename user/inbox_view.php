...
<body>
  ...
  <div class="container">
    <div class="row mt-3">
      <div class="jumbotron col-md-12" style="padding: 50px; padding-left: 50px;">
        ...
        <a href="user_index.php?page=ib" style="text-decoration: none; color: black; font-weight: 400;">
        ...
        <span style="margin-left: 10px; font-size: 25px; font-weight: 370;">Inbox</span>

        <form action="" method="post">

          <h3 style="margin-top: 25px; margin-bottom: 15px;" id="sub"><?php echo $sub ?></h3>
          <span style="font-weight: 500; font-size: 23px;" id="name"><?php echo $name[0] ?></span> &nbsp; <span id="from" style="font-weight: 370; font-size: 23px;"><?php echo "~" . $col[1] ?></span>
          <h6 style="font-weight: 400;" id="date"><?php echo $col[6] ?></h6>
          <h6 style="font-weight: 500;" id="to"><?php echo 'To ' . ' ~' . $col[2] ?></h6>
          ...
          <a href="user_index.php?page=cm&mid='<?php echo $col[0] ?>'&act=rep" class="btn btn-outline-success" style="border: none;">
  ...
  if (isset($_POST['del'])) {
    ...
    // echo $mid;
    // echo $col[5];
    $select = "update mail set dt='y' where mid='$mid'";
    mysqli_query($connect, $select) or die("hello");
    if ($result) {
      echo "<script>alert('Mail Deleted Successfully');</script>";
      echo "<script>window.location='user_index.php?page=ib'</script>";
    }
  }
